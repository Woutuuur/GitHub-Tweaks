...
function createCopyButtonHtml(fileName) {
    return (
        `<clipboard-copy data-copy-feedback="Copied!" aria-label="Copy" value = ${encodeHTML(fileName)} data-view-component="true" class="Link--onHover color-fg-muted ml-2 mr-2" tabindex="0" role="button">
...
function addFilenameCopyButton(request, sender, sendResponse) {
    ...
    if (request.type !== "tabUpdateEvent") return
    if (!request.tab.url.match(pattern)) return

    const fileName = document.getElementsByClassName("final-path")[0].innerHTML;
    const filePathElements = document.querySelector("#blob-path");
    const copyButton = document.createElement("span");
    const element = createCopyButtonHtml(fileName)

    copyButton.innerHTML = element
